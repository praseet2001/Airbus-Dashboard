...
all_tool_type = sorted(df["BOT/COT/DOT"].dropna().unique())
...
# c1, c2, c3 = st.columns([1.0, 1.5, 0.4])
...
tool_type_filter = c3.multiselect(
    "Tool Classification",   # 💼 executive-friendly naming
    all_tool_type,
...
with t2:
    st.markdown("### 🧩 Tool Classification")


# ✅ VALUES ROW (ALIGNED JUST BELOW TITLES)
...
with c2:
    ...
    k4.metric("BOT", (filtered_df["BOT/COT/DOT"] == "BOT").sum())
    k5.metric("COT", (filtered_df["BOT/COT/DOT"] == "COT").sum())
    k6.metric("DOT", (filtered_df["BOT/COT/DOT"] == "DOT").sum())
# =================================================
# DONUT + TOOL DETAILS
# =================================================
...
for i, cat in enumerate(categories):
    with cat_cols[i]:
        ...
        st.plotly_chart(fig_small,config={"responsive": True})
...
with left_map:
    ...
    st.plotly_chart(fig_map, config={"responsive": True})
